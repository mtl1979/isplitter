...
HRESULT RegisterHandlerClass(const CLSID& clsid, PCWSTR pszClassName, PCWSTR pszFriendlyName)
{
	...
	hr = SetHKCRRegistryKeyAndValue(pszClassName, NULL, pszFriendlyName);
	if (SUCCEEDED(hr))
	{
		hr = StringCchPrintf(szSubkey, ARRAYSIZE(szSubkey),
			L"%s\\CLSID", pszClassName);
		if (SUCCEEDED(hr))
		{
...
HRESULT UnregisterHandlerClass(PCWSTR pszClassName)
{
	if (pszClassName == NULL)
	{
		return E_INVALIDARG;
	...
	hr = HRESULT_FROM_WIN32(RegDeleteTree(HKEY_CLASSES_ROOT, pszClassName));
